...
fun SignUpScreen(viewModel: SignUpViewModel) {
    ...
    // SignUpScreen: 이 함수의 이름입니다. 이 함수는 회원가입 화면의 UI를 정의합니다.
    // (viewModel: SignUpViewModel): 이 함수의 매개변수입니다.
    //     SignUpScreen은 SignUpViewModel 타입의 'viewModel' 객체를 받습니다.
    //     이는 MVVM 패턴에 따라 View(SignUpScreen)가 ViewModel로부터 데이터를 받고 사용자 이벤트를 전달하기 위함입니다.

    Column(
        // Column: 자식 Composable들을 세로 방향으로 순서대로 배치하는 레이아웃 컴포저블입니다.
        // 회원가입 화면의 요소들(텍스트, 버튼 등)을 세로로 정렬하기 위해 사용됩니다.
        modifier = Modifier
        ...
        // text = "Sign Up": 표시될 텍스트 내용입니다. 회원가입 화면의 제목 역할을 합니다.

        Button(onClick = { viewModel.signUpWithGoogle() }) {
            // Button: 클릭 가능한 버튼 UI 컴포넌트입니다.
            // onClick = { ... }: 버튼이 클릭되었을 때 실행될 람다(익명 함수)입니다.
            //     viewModel.signUpWithGoogle(): ViewModel에 정의된 'signUpWithGoogle' 함수를 호출합니다.
            //     이 함수는 Google을 통한 회원가입 비즈니스 로직(현재는 스텁)을 처리합니다.
            Text("Sign up with Google (Stub)")
            // Text: 버튼 안에 표시될 텍스트 내용입니다. 이 버튼이 Google 회원가입 기능을 제공함을 나타냅니다.
            // "(Stub)"은 아직 실제 구현이 아닌 임시 기능임을 명시합니다.
        }
        Button(onClick = { viewModel.signUpWithNaver() }) {
            // Button: Google 회원가입 버튼과 동일하게 작동하는 Naver 회원가입 버튼입니다.
            // onClick = { viewModel.signUpWithNaver() }: Naver를 통한 회원가입 비즈니스 로직(현재는 스텁)을 호출합니다.
            Text("Sign up with Naver (Stub)")
            // Text: 버튼 안에 표시될 텍스트 내용입니다. 이 버튼이 Naver 회원가입 기능을 제공함을 나타냅니다.
        }
        // TODO: Add actual sign-up UI elements
        // TODO 주석: 개발자가 나중에 구현해야 할 작업이나 기능을 표시하는 표준 주석입니다.
        //     이 부분에 실제 회원가입에 필요한 입력 필드(이름, 이메일, 비밀번호 등), 다른 회원가입 옵션 등의 UI 요소가 추가될 예정입니다.
    }
}
